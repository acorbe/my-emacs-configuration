...
** Moving text-mode
Moves blocks of text up and down with M-up/down
#+BEGIN_SRC emacs-lisp

  (use-package move-text
  :straight t
  :config (progn (move-text-default-bindings)
...
This seems a very nice package but conflicts with magit when committing
#+BEGIN_SRC emacs-lisp
  ...
  ;;   ;; I prefer these commands to be bound to word movement than to splicing
...
#+BEGIN_SRC emacs-lisp
  ...
  :hook (after-init . doom-modeline-mode)
  :init
  ;; Set variables early, before loading
  (setq doom-modeline-height 25)
  (setq doom-modeline-icon (display-graphic-p))
  (setq doom-modeline-major-mode-icon t)
  (setq doom-modeline-enable-word-count t)
  :config
  ;; Redundant but harmless here; package is fully loaded
...
My custom snippets
#+begin_src emacs-lisp
(let ((snippet-dir "~/.emacs.d/snippets/python-mode/")
      (snippet-file "as"))
  (make-directory snippet-dir t)
  (with-temp-file (expand-file-name snippet-file snippet-dir)
    (insert "# -*- mode: snippet -*-\n")
    (insert "# name: assign from rhs\n")
    (insert "# key: as\n")
    (insert "# --\n")
    (insert "${1:`(let* ((rhs (yas-text))\n")
    (insert "            (clean (car (split-string rhs \"(\"))))\n")
    (insert "        clean)`} = $1\n")))
#+end_src
...
#+BEGIN_SRC emacs-lisp
    ...
    (defvar-local my/latex-image-dir nil
      "Buffer-local directory to store pasted images for LaTeX documents.")

    (defun my/paste-image-from-clipboard-to-tex ()
      "Paste clipboard image into buffer-local image directory and insert LaTeX includegraphics line.
    Prompts for image file name (default: timestamp). Prompts for image directory only once per buffer."
      (interactive)
      ;; Ask for image directory only once per buffer
      (unless my/latex-image-dir
        (setq my/latex-image-dir
              (read-directory-name "Image directory for this document: " "./Figures/"))
        (unless (file-directory-p my/latex-image-dir)
          (make-directory my/latex-image-dir t)))

      ;; Prompt for filename, defaulting to timestamp
      (let* ((default-name (format-time-string "img_%Y%m%d_%H%M%S"))
             (input-name (read-string (format "Image file name (default: %s): " default-name)))
             (base-name (if (string-empty-p input-name) default-name input-name))
             (file-name (concat base-name ".png"))
             (image-path (expand-file-name file-name my/latex-image-dir))
             (relative-path (file-relative-name image-path (file-name-directory (buffer-file-name)))))
        ...
        (cond
         ...
         ;;  (call-process-shell-command (for;; mat "xclip -selection clipboard -t image/png -o > %s" image-path)))

         ;; ((eq system-type 'gnu/linux)
         ;;  (if (yes-or-no-p "Compress image before saving?")
         ;;    ;; Use ImageMagick's convert
         ;;    (call-process-shell-command
         ;;     (format "xclip -selection clipboard -t image/png -o | convert png:- -strip -resize 70%% -quality 90 %s" image-path))
         ;;  ;; Raw save
         ;;  (call-process-shell-command
         ;;   (format "xclip -selection clipboard -t image/png -o > %s" image-path))))
  	 ((eq system-type 'gnu/linux)
  	  (if (not (executable-find "xclip"))
  	      (error "xclip is not installed or not in PATH. Please install it to paste images.")
  	    ;; If xclip is available, continue
  	    (let* ((format-choice (completing-read "Save image as (png/jpg): " '("png" "jpg") nil t "png"))
  		   (ext format-choice)
  		   (image-path (expand-file-name (concat base-name "." ext) my/latex-image-dir))
  		   (relative-path (file-relative-name image-path (file-name-directory (buffer-file-name)))))
  	      (if (yes-or-no-p "Compress image before saving?")
  		  ...
  		  (call-process-shell-command
  		   (format "xclip -selection clipboard -t image/png -o | convert png:- -strip -resize 70%% -quality 90 %s" image-path))
  		;; Raw PNG only
  		(if (string= format-choice "png")
  		    (call-process-shell-command
  		     (format "xclip -selection clipboard -t image/png -o > %s" image-path))
  		  ;; For JPG, xclip gives PNG → convert
  		  (call-process-shell-command
  		   (format "xclip -selection clipboard -t image/png -o | convert png:- %s" image-path))))
  	      (insert (format "\\includegraphics[width=\\linewidth]{%s}" relative-path)))))
         
         ;; Windows
         ((eq system-type 'windows-nt)
          (call-process-shell-command
           (format "powershell -command \"Add-Type -AssemblyName System.Windows.Forms; [Windows.Forms.Clipboard]::GetImage().Save('%s')\"" image-path))))

        ;; Insert LaTeX includegraphics
        (insert (format "\\includegraphics[width=\\linewidth]{%s}" relative-path))
  	))


  (defun my/set-latex-image-dir ()
    (interactive)
    (setq my/latex-image-dir
            (read-directory-name "Image directory for this document: " "./Figures/"))
      (unless (file-directory-p my/latex-image-dir)
  ...
  (defun my/insert-figure-from-file-to-tex ()
    "Insert an existing image into LaTeX document by copying it into a buffer-local image directory.
  User is prompted to choose the image file and optionally rename it.
  Then \\includegraphics is inserted with relative path."
    (interactive)
    ;; Set image directory once per buffer
    ;; this should call the function above
    (unless my/latex-image-dir
      (setq my/latex-image-dir
            (read-directory-name "Image directory for this document: " "./Figures/"))
      (unless (file-directory-p my/latex-image-dir)
    ...
    ;; Ask for existing image file
    (let* ((source-path (read-file-name "Select image file: "))
           (ext (file-name-extension source-path))
           (default-name (file-name-base source-path))
           (input-name (read-string (format "New file name (default: %s): " default-name)))
           (base-name (if (string-empty-p input-name) default-name input-name))
           (target-filename (concat base-name "." ext))
           (target-path (expand-file-name target-filename my/latex-image-dir))
           (relative-path (file-relative-name target-path (file-name-directory (buffer-file-name)))))

      ;; Copy file to local figure directory
      (copy-file source-path target-path t) ;; t = overwrite without prompt

      ;; Insert LaTeX includegraphics line
      (insert (format "\\includegraphics[width=\\linewidth]{%s}" relative-path))))
